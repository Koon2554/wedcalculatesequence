...
function addInput() {
    ...
    div.innerHTML = `<input type="text" placeholder="Enter your value...">
...
function showValues() {
    ...
    document.getElementById('valuesList').innerText = "Entered values: " + values.join(', ');
}
...
function find_n_poly_div_sequence(num, parameter) {
    let log_sum = Math.log(parameter[0]);

    for (let i = 1; i < parameter.length; i++) {
        log_sum += binomial(num.n - 1, i) * Math.log(parameter[i]);
    }
...
function input_1() {
    ...
    if (line === "" || nValue === "") {
        alert("Please enter all data before calculating!");
    } else {
        ...
        if (num.is_harmoni) {
            if (num.how_to === 1) {
                let formulaText = `
                Ans : 1/${res} <br>
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                From the formula: <br>
                $$a_n = a_1 + (n - 1)d $$ 
                From observing the sequence, it can be seen that the common difference is ${num.data[1][0]} <br>
                The first term is  ${num.data[0][0]} <br>
                The term to be found is  ${num.n} <br>
                ...
                if (count === 0) {
                    ...
                    document.getElementById('run').innerHTML = "Sorry for the inconvenience due to some errors, such as calculating very high values until they become infinity or finding null values, making it impossible to calculate.";
                }
                MathJax.typeset();
                ...
                Ans : 1/${res} <br>
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                From the formula $$a_n = a_1 \\times r^{(n-1)} $$
                From observing the sequence, it can be seen that the common ratio is  ${num.data[1][0]} <br>
                The first term is  $$a_1 = ${num.data[0][0]}$$ 
                The term to be found is  n = ${num.n} <br>
              ...
              `;
                ...
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                From the formula: <br>
                $$ a_n = a_1 + \\sum_{k=0}^{n-1} \\binom{n-1}{k} \\cdot d_k $$
                ...
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                From the formula: <br>
                $$ a_n = a_1 \\cdot \\prod_{k=0}^{n-1} d_k^{\\binom{n-1}{k}} $$
                ...
                The resulting value:
                $$ a_n = ${num.data[0][0]} \\times ${product / num.data[0][0]} = ${product} $$
                `;
                data += res;
                data += num.n;
                data += num.data[0][0];
                ...
                Ans : 1/${res} <br>
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                From the formula: $$ a_n = a_{n-1} + a_{n-2} $$ 
                This sequence is a recursive sequence because each term depends on the previous terms.<br>
                Initial terms: $$ a_1 = ${num.data[0][0]}, a_2 = ${num.data[0][1]} $$ 
                The term to be found: $$ n = ${num.n} $$
                Therefore, the ${num.n}th term in the sequence above is ${res}
                `;
                ...
                It will be noted that the sequence obtained is a harmonic sequence, so only the denominators are analyzed. <br>
                <div>
                  <p><b>Sequence Analysis Result:</b></p>
                  <p>
                    The analysis checks the sequence layer by layer, using <b>addition</b> for sequences with a constant difference 
                    and <b>multiplication</b> for sequences with a constant ratio, to find the relationship between terms.
                  </p>
                
                  <p><b>Calculation Steps:</b></p>
                  ${expandedFormula}
                  <p>
                    From these steps, the sequence pattern can be identified and an equation describing the general term can be created.
                  </p>
                </div>
                `;
                let data = [res];
                ...
                document.getElementById('run').innerHTML = "Ans : 1/" + res + "\nThe given sequence may be outside the scope of study. The result is calculated from Lagrange polynomial.";
            } 
        } else {
            if (num.how_to === 1) {
                let formulaText = `
